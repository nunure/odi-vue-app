<template>
  <div class="carousel">
    ...
    <i
      ...
      @click.prevent="next"/>
    <button
      class="carousel__nav carousel__next btn-circle btn-default "
      @click.prevent="next">
      <i class="fas fa-chevron-right fa-3x"/>
    </button>
    <button
      class="carousel__nav carousel__prev btn-circle btn-default"
      @click.prevent="prev">
    ...
    next() {
      if (this.$parent.$refs.vfg[this.index].validate()) {
        ...
        if (this.index === this.slidesCount - 1) {
          ...
          this.$http
            .put("http://localhost:3000/answers", this.$parent.answer)
            .then(
              response => {
                // success callback
